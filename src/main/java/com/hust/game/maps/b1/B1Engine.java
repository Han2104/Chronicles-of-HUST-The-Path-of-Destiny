...
public class B1Engine {
    ...
    // =========================================================
    public enum State {
        PLAYING, QUIZ_MODE, DEBUFF_QUIZ_MODE, GAME_OVER, VICTORY
    }
    ...
    private static final int WRONG_ANSWER_ENERGY_PENALTY = 1;
     ...
     * @param aimDX,             aimDY hướng bắn (world coords từ cursor - player
     *                           position)
     */
    public void update(boolean up, boolean down, boolean left, boolean right,
        ...
        B1CollisionManager.CollisionResult result = collisionManager.checkCollisions(player, boss,
                bulletManager.getBullets(), shieldOn);
    ...
    public void answerQuiz(int selectedIndex) {
        ...
        if (correct) {
            ...
            p.setEnergy(p.getEnergy() - WRONG_ANSWER_ENERGY_PENALTY);
            System.out.println("❌ Trả lời sai! -" + dmg + " HP, -" + WRONG_ANSWER_ENERGY_PENALTY + " Energy");
        }

        checkEndConditions();
        // Nếu game vẫn chạy → về PLAYING
        if (currentState == State.QUIZ_MODE)
    ...
    public void answerDebuffQuiz(int selectedIndex) {
        ...
        checkEndConditions();
        if (currentState == State.DEBUFF_QUIZ_MODE)
            currentState = State.PLAYING;
    }
    ...
    public boolean activateItem(B1ItemManager.ItemType type) {
        ...
        if (!success)
            pendingWarning = "Vật phẩm đang hồi chiêu!";
        return success;
    }
    ...
    private boolean checkItemRequirement(B1ItemManager.ItemType type, Player p) {
        switch (type) {
            case GIAO_TRINH_OOP:
                return p.getSolutionSkill() >= 70;
            case SO_GHI_CHEP:
                return p.getDisciplineScore() >= 80;
            case HOC_BONG_KKHT:
                return p.getLevel() >= 8 && p.getFinance() >= 100;
            default:
                return false;
        }
    }
    ...
    private void clampPlayerToBounds() {
        int margin = 5;
        if (player.getX() < margin)
            player.setX(margin);
        if (player.getX() > ARENA_W - 30 - margin)
            player.setX(ARENA_W - 30 - margin);
        if (player.getY() < margin)
            player.setY(margin);
        if (player.getY() > ARENA_H - 30 - margin)
            player.setY(ARENA_H - 30 - margin);
    }
    ...
    public B1Player getPlayer() {
        return player;
    }

    public B1Boss getBoss() {
        return boss;
    }

    public List<B1Bullet> getBullets() {
        return bulletManager.getBullets();
    }

    public D9QuestionBank.Question getCurrentQuizQuestion() {
        return currentQuizQuestion;
    }

    public B1ItemManager getItemManager() {
        return itemManager;
    }

    public B1DebuffManager getDebuffManager() {
